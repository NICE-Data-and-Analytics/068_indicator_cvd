...
load_data <- function(con, project_id, n, filename, cohort_def_var, grouping_var, categories) {
    
    # Create row counter to sum all rows read
    if (!exists("counter")) {
        counter <- 0
    }
    ...
    dbExecute(con, glue("CREATE TEMPORARY TABLE {project_id}_df LIKE {project_id}_temp;"))
    ...
    cohort_only_query <- glue("INSERT INTO {project_id}_df (patid, pracid, obsid, obsdate, medcodeid, value, numunitid, obstypeid)
        SELECT patid, pracid, obsid, obsdate, medcodeid, value, numunitid, obstypeid FROM {project_id}_temp
        WHERE patid IN (
            SELECT patid
    ...
    temp_df_rows <- dbGetQuery(con, glue("SELECT COUNT(*) FROM {project_id}_df;")) %>% slice(1) %>% pluck(1)
    
    counter <- counter + temp_df_rows
    
    # Drop temporary table
    dbExecute(con, glue("DROP TABLE {project_id}_temp;"))
    
    # Keep latest record per patient
    latest_rec_query <- glue("CREATE TABLE latest
        ...
        r AS ( -- Calculate latest record for each patient and grouping
            SELECT *,
                ROW_NUMBER() OVER (PARTITION BY {grouping_var} ORDER BY obsdate DESC) AS rn
            FROM u
        )
        SELECT patid,
            pracid,
    ...
    # Replace {project_id}_{cohort_def_var}_define with latest
    dbExecute(con, glue("DROP TABLE {project_id}_{cohort_def_var}_define;"))
    
    dbExecute(con, glue("ALTER TABLE latest RENAME {project_id}_{cohort_def_var}_define;"))
...
# Load codelist
codelist_file <- "bmival_cprd_plus.txt"
...
dbGetQuery(con, "SELECT DISTINCT cluster_id FROM p068_codes;")
```
...
filename <- "obesity_Define_Inc1_Observation"
grouping_var <- paste0(c("patid", "cluster_id"), collapse = ", ")
categories <- paste0(paste0("'", c("Obese", "Not obese"), "'"), collapse = ", ")
...
walk(file_seq, \(x) load_data(n = x, project_id = "p068", con = con, filename = filename, cohort_def_var = cohort_def_var, grouping_var = grouping_var, categories = categories))
```
